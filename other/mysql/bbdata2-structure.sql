...
CREATE TABLE IF NOT EXISTS `stats` (
  `object_id` INT(11) NOT NULL,
  `n_reads` INT NULL DEFAULT 0,
  `n_writes` INT NULL DEFAULT 0,
  `last_ts` TIMESTAMP NULL,
  `avg_sample_period` DOUBLE NULL DEFAULT 0,
...
USE `bbdata2`$$
DROP TRIGGER IF EXISTS `ugrps_AFTER_INSERT` $$
USE `bbdata2`$$
CREATE TRIGGER `ugrps_AFTER_INSERT` AFTER INSERT ON `ugrps` FOR EACH ROW
BEGIN
	-- we make ROOT an admin and then we block its removal/modification in the API code
	INSERT IGNORE INTO users_ugrps (user_id, ugrp_id, is_admin) VALUES (1, NEW.id, 1);
	-- rebuild_mat_view will be called upon insert
END$$
